            ...
            @foreach( $votes as $vote )
                <div class="my-4">
                    <div
                        ...
                        <span class="dark:text-primary-light">{{ $vote->name }}</span>
                        <span
                            class="flex flex-row px-3 py-1 justify-between items-center text-sm font-bold text-gray-100 transition-colors duration-200 transform rounded cursor-pointer {{ $vote->color($vote->type) }}">{{ $vote->type === 'cubi' ? __('vote.create.type_cubi') : __('vote.create.type_virtual', ['currency' => config('pw-config.currency_name')]) }}
                        </span>
                    </div>
                    <div
                        ...
                        @if( $vote_info[ $vote->id ]['status'] )
                            <form action="{{ route('app.vote.check', $vote->id )  }}" method="post">
                                {{ csrf_field() }}
                                <x-hrace009::button type="submit">
                                    {{ __('vote.button', [ 'name' => $vote->name ]) }}
                                </x-hrace009::button>
                            </form>
                        @else
                            <div class="text-center">
                                <span>{{ __('vote.cooldown') }}</span>
                                <div data-countdown="{{ $vote_info[ $vote->id ]['end_time'] }}"></div>
                            </div>
                        @endif
                    </div>
                </div>
